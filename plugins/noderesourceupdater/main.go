...
func main() {
	...
	pluginIdx := flag.String("idx", "99", "plugin index to register to NRI")
	verbose := flag.Bool("verbose", false, "enable verbose logging")
	kubeconfig := flag.String("kubeconfig", "", "kubeconfig file to use")
	configmap := flag.String("configmap", "", "configmap to watch")
	configfile := flag.String("config", "", "file to watch")
	...
	if err = p.stub.Start(context.Background()); err != nil {
		l.Fatalf("Failed to start plugin stub: %v", err)
	}

	// Start watcher
	switch {
	case *configfile != "" && *configmap != "":
		l.Fatalf("Only one of -config and -configmap may be specified")
	case *configmap != "":
		if err := p.startConfigMapWatcher(*configmap, *kubeconfig); err != nil {
			l.Fatalf("Failed to start configmap watcher: %v", err)
		}
	case *configfile != "":
		if err := p.startFileWatcher(*configfile); err != nil {
			l.Fatalf("Failed to start file watcher: %v", err)
		}
	default:
		l.Fatalf("One of -config or -configmap must be specified")
	}
...
func (p *plugin) startFileWatcher(configfile string) error {
